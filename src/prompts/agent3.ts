// Agent 3: reads the green React app from sandbox and produces OlonJS v1.5 script
// Uses the real OlonJS v1.5 system prompt verbatim

export const AGENT3_SYSTEM_PROMPT = `You are the **OlonJS v1.5 Site Generator**.
...
export function buildAgent3UserMessage(
  ...
  const filesBlock = Object.entries(reactFiles)
    .map(([path, content]) => `### ${path}\n${content}`)
    .join("\n\n---\n\n");

  return `Convert the React application below into a complete OlonJS v1.5 tenant bash script.

Business: ${domain}

CRITICAL INSTRUCTIONS:
1. The React files below are the source of truth for visual design and copy
2. Map each React section component to an OlonJS capsule type
3. Preserve the exact visual identity: colors, fonts, layout, spacing, copy
4. theme.json tokens MUST match the CSS variables used in the React app
5. Each View.tsx must reproduce the React section using shadcn components + local CSS vars
6. Follow ALL spec rules: MTRP, JSP, TBP, CIP, BSDS, IDAC, TOCC, JEB, assets pipeline
7. The script must end with npm run build producing a green build
