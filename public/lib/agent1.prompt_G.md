...
The script may:
- run the required shadcn/ui setup commands
- write files only under `src/**`
- write `index.html`

The script must not generate or modify infrastructure or tenant system files outside that scope.
In particular it must never write:
- `vite.config.*`
- `package.json`
- `tsconfig*`
- `vercel.json`
- `public/**`
- root scripts or helper files other than the single generated bash script itself


The script uses only heredoc syntax (`cat > file << 'EOF'`) for file generation and runs inside the root of an existing OlonJS project (React 19 + TypeScript + Vite + Tailwind v4 + @olonjs/core).
...
- if the component computes a normalized source such as `navItems`, it must render `navItems`
- do not compute resolved menu data and then render the raw `menu` prop directly
- desktop nav should use shadcn `NavigationMenu` when appropriate
- mobile nav should use shadcn `Sheet` when appropriate

Card and surface rule:
- use shadcn `Card` for card-like sections when it matches the structure
- card surfaces must still flow through tenant tokens
- prefer patterns like `bg-[var(--local-surface)] border-[var(--local-border)] text-[var(--local-text)]`
- do not rely on shad default card colors if the section owns the visual surface

Forms and controls:
- use shadcn form primitives for inputs, labels, selects, switches, checkboxes, textareas, and toggles
- input, border, focus, and surface styling must still follow tenant token semantics
- do not hardcode `bg-slate-*`, `text-zinc-*`, `border-white/10`, or similar shortcuts for tenant-owned controls

Color and token chain rule:
- tenant-owned UI colors must always flow through the token chain
- canonical chain: `theme.json -> runtime vars -> tenant semantic bridge -> --local-* -> JSX classes`
- do not bypass the tenant contract with hardcoded utility colors for primary themed UI
- do not use `bg-primary`, `text-primary`, `text-white`, `bg-blue-600`, `border-white/10`, `bg-slate-950`, or `text-zinc-*` as the primary styling contract for tenant-owned UI
- do not use fallback expressions such as `var(--color-accent, #...)` for primary themed concerns

Forbidden patterns:
- raw CTA buttons styled by hand when shadcn Button is the correct primitive
- `variant="accent"`, `variant="brand"`, `variant="primary"`, `variant="cta"`
- hardcoded radius pixels for primary themed UI
- hardcoded slate/zinc/white utility colors for section-owned themed concerns
- rendering raw `menu` when a normalized menu source has already been computed
- treating shadcn defaults as a replacement for tenant theme semantics
